...
var cities = [
				...
				{'loc':[40.104516, -88.228238] , 'name':'Urbana-Champaign','code':'IL'},
...
function init_render(){
	...
	init_center = [37.235613, -22.942755];
...
function onMarkerClick(e) {
    ...
    cities.forEach(function(element) {
        if (clickedLatlng[0] == element.loc[0] && clickedLatlng[1] == element.loc[1]){
			...
			render_data(dataPoints,[dataPoints[0].latitude , dataPoints[0].longitude],false);
        }
        else{/*
	...
	if (is_init==true){
		...
		cities.forEach(function(city) {
			if (selectedCity != city.name)
				dataPoints.push({'latitude':city.loc[0],'longitude':city.loc[1],'name':city.name});
		});
	}
	...
	//if (is_init == true){
		for (i = 0; i < cities.length; i++) {
			var textDisplay = cities[i].name,	//value searched
				location = cities[i].loc;		//position found
